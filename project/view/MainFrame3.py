...
class MainFrame(wx.Frame):
    ...
    def on_verify(self, e):
        ...
        for k, v in self.processorPanels.items():
            processors.append(Processor(v.editPname.GetValue(), v.policyCombo.GetValue(), v.preemptCombo.GetValue()))
        bus = Bus(self.busPanel.edit_bcet.GetValue(), self.busPanel.edit_wcet.GetValue())
        tasks = []
        deps = []
        for k, v in self.taskPanels.items():
            tasks.append(Task(v.editTid.GetValue(), v.editOffset.GetValue(), v.editBcet.GetValue(), \
                              v.editWcet.GetValue(), v.editDeadline.GetValue(), v.editPeriod.GetValue(),\
                              v.editProcessor.GetValue()))
            if len(v.editPre.GetValue()) != 0:
                deps.append(Dep(v.editTid.GetValue(), v.editPre.GetValue()))
